...
def export_default_file_csv(table, name_websie, province, type ,limit_number = 10):
    ...
    df = pd.read_sql_query('select title,detail_address,title_address,price,unit_price,bedroom,toilet,law,indoor,road,face_first,floor,direction_balcony,description,area,unit_area,images,type_new,direction_of_house,categoty,created_at,exp_at from ' + table +' limit ' + str(limit_number), cnx)
